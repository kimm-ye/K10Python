...
모듈
    ...
    파일로 함수나 변수 또는 클래스를 모아놓은 것을 말한다.
...
# 모듈 사용방법2 : 이와같이 하면 함수 호출시 모듈명을 생략할 수 있다. (from 사용하면 함수명을 import)
...
'''
__name__ 변수를 사용한 모듈로 이와 같이 import를 하게 되면 
"mod2"가 저장된다. (메인부분은 실행되지 않음)
'''
import mod2
result = mod2.mul(3,4)
print(result)
...
# 이와같이 하면 함수명 만으로 호출이 가능하다. (from 사용)
