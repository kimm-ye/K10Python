...
예외처리
    : 에러를 핸들링하기 위해 사용된다.
    파이썬에서는 try ~ except 를 사용한다.
    그리고 else를 사용할 수 있다.
    
    else는 예외가 발생하지 않았을때 실행할 코드를 기술한다.
    finally는 예외 발생여부와 상관없이 항상 실행할 코드를 기술한다.
'''
...
calc([10,20])   #인덱스 에러
print('실행3')
calc([100,101,102,103])  #NameError
print('실행4')
...
try:
    ...
    try:
        ...
        print("파일 객체를 닫습니다.")
        fp.close()
except IOError: # 현재 경로에 파일이 없으므로 에외발생
    print('파일에러발생')
