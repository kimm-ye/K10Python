...
print("sum(10,20)호출 후 반환값=", sum(10,20)) #합의 결과 30이 출력됨
print("============================")
...
def defaultArgs(lan = "Java"):
    ...
    return
defaultArgs() #자바에서는 에러이지만 이 경우 디폴트 값인 java가 출력된다.
defaultArgs("C++") #여기서는 C++이 출력됨
# 만약 Java에서의 호출이라면 첫번째 호출에서 에러가 발생하게 될 것이다.

# 4. 가변매개변수 : 여러개의 값을 한꺼번에 받고 싶을때 사용
'''
4. 가변매개변수
    : 여러개의 매개변수를 전달해야 할때 사용하는 매개변수로
    Java의 가변인자와 비슷하다.
    *를 이용해서 매개변수값을 튜플로 그룹화한다.
    **를 이용해서 매개변수를 딕셔너리로 사용할 수 있다.
    
    가변인자(C:\02WorkspaceKYJ\K01JAVA\src\ex12inheritance) 참고
'''
def product(*args):
    print("*args=>", args) # 튜플 형태로 출력된다.
    result = 1
    for a in args: # 튜플이므로 반복문 사용가능
        result *= a #각 원소를 통해 누적합을 계산함
