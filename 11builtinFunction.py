...
'''
print("="*5, "sorted()", "="*20)
numbers = (8,7,6,8,4,9,7,5,3,2,7,4,9,8,2,6,5) #튜플로 데이터를 생성 (소괄호니까)
print(sorted(numbers)) # 정렬결과는 리스트로 반환
...
이터레이터(iterator)
    ...
    iter() : 반복을 끝낼 값(sentinel)을 지정하면 특정값이 나올때 반복을 
        종료하는 함수.
        형식]
...
while True: #무한루프
    try:
        print('it=', next(it)) #반복할 항목이 있을때 하나씩 반환
...
제너레이터 함수
    ...
    yield는 return과 같은 역할을 하기 때문에 따로 쓸일은 없고, 난수생성 같은데 쓰인다.
'''
print("="*5, "제너레이터 함수1", "="*20)
print('yield의 동작 과정')
def number_generator1(): #제너레이터 함수 정의
    yield 0  #첫번째 next()를 통해 0을 함수밖으로 반환(양보)한다.
...
def number_generator2(n):
    i = 0
    while i<n:
        yield i #yield가 실행될때 현재 i값이 for문으로 전달되어 출력된다. (여기서 약간 return과 같은 역할)
        i += 1
...
for i in upper_generator(fruits): #리스트의 인자만큼 반복호출된다.
    print(i, end=' ')
