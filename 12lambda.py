'''
람다(lambda)함수
    : def 키워드를 사용하지 않고 식 형식으로 되어있다해서 람다식이라고 부른다.
    이름이 없으므로 익명함수로 부르기도 한다. lambda 키워드를 사용해서
    간편하게 작성할 수 있고, 재사용되지 않는 1회성 함수를 만들때 사용한다.
'''
# 두 수의 합을 일반적인 함수로 정의 
def two_sum(x,y):
    return x + y
...
'''
map()함수
    : 전달된 데이터를 동일함수에 반복적으로 적용시켜 주는 역할을 한다.
...
#list_data의 원소 갯수만큼 람다식을 반복호출하여 얻어진 결과를 리스트로 반환한다.
...
형식]
    ...
    - elif를 사용할 수 없다. 따라서 2개 이상의 조건도 if를 연속으로 사용해야한다.
'''
print("### 람다식과 map함수2 ###")
...
reduce 함수
    ...
    누적해서 반환하는 함수.
    파이썬 3부터 내장함수가 아니므로 functools모듈을 import한 후
    사용해야 한다.
'''
print("### 람다식과 reduce함수 ###")
...
'''
    1~10까지 합을 반환한다. 람다식이 두 요소를 더해서 반환하도록
