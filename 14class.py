...
클래스메소드 
    ...
    self대신에 cls라는 클래스를 의미하는 파라미터를 전달받는다.
    cls를 통해 클래스변수에 엑세스할 수 있다.
    @classmethod 라는 데코레이터를 사용한다.
'''
class CalculatorInit:
    
    #클래스 변수 (클래스 전체에서 접근가능한 지역변수)
    count = 0  
    
    #생성자 오버로딩이 되지 않음. 자바는 생성자 인자 몇개씩 해도 상관이 없으나 파이썬은 되지 않음.
    '''
    파이썬에서는 Java 처럼 생성자 오버로딩은 지원하지 않는다.
    ...
    #생성자 메서드 정의. 멤버변수 초기화를 위해 첫번째 파라미터에 self를 기술해야한다.
...
#fCal2 = CalculatorInit() -> 인자가 없는 기본생성자는 정의되지 않는다. (에러발생)

# 2개의 인자를 전달해서 객체 생성
fCal = CalculatorInit(2010, 43)
# 일반적인 인스턴스 메서드 호출
print(fCal.addition())
# 클래스 메서드 호출1 (클래스 메서드는 static과 비슷) : 인스턴스 변수를 통한 호출
fCal.showInfo() 
# 클래스 메서드 호출2 : 클래스명을 통한 호출
CalculatorInit.showInfo() 

# 정적메서드 호출 (당연히 클래스명을 통해서 호출하는게 가능)
...
print("자식객체로호출", p2.addition()) # 결과 : 600 (오버라이딩해서 *2를 했으므로)
