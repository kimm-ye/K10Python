...
open()
    ...
    mode : 파일 열기 모드
        r : 읽기모드
        w : 쓰기모드
        a : 추가모드. 파일이 마지막 부분에 새로운 내용을 추가한다.
        b : 2진모드. 바이너리모드 라고도 하는데, 컴퓨터가 알아볼 수 있는 파일을 생성한다.
        t : 텍스트모드로 기본값이다. 사람이 알아볼 수 있는 형태의 텍스트 파일을 생성한다.
            차후 메모장으로 열 수 있다.
'''
...
while True:
    ...
    if not line : break #더이상 읽을 내용이 없다면 반복문을 탈출한다.
    print(line) #출력
f_read.close()
...
# kosmo.p 라는 파일을 2진파일 쓰기모드로 오픈한다.
...
# 2진파일 읽기모드로 파일을 오픈한 후 load()를 통해 복원한다. (2진파일이라 열려도 볼순없다)
